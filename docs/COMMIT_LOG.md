...
- Hash: `TBD`
- 變更範圍: `ActivityForm.vue`, `CollectionForm.vue`, `mapUtils.spec.ts`
- 詳細內容:
  - **表單優化**: 為 `ActivityForm` 和 `CollectionForm` 新增「閱覽/編輯」切換模式。在閱覽模式下，所有輸入框轉為易讀的純文字，且網址（地圖、來源）變為可直接點擊開啟的連結，大幅提升資訊查閱效率。
  - **測試強化**:
    - **表單測試**: 為兩個表單的「閱覽/編輯」模式新增了完整的單元測試，涵蓋初始狀態、模式切換、以及不同模式下的 UI 渲染正確性。
    - **mapUtils 測試**: 全面重寫 `mapUtils.spec.ts`，導入 12 個涵蓋優先級判斷、`null`/`undefined` 等邊界值的測試案例，確保地圖連結生成邏輯的魯棒性。
  - **品質工程**: 修正因新功能引入而導致的既有測試案例失敗問題，並清除所有相關的 Vue 警告。
...
- Hash: `TBD`
- 改動方向: 支援非均攤分帳情境，校準視覺風格至木質調，並提升組件測試覆蓋率。
- 具體內容:
  - **自訂分帳功能**:
    - 更新 `ExpenseSchema` 支援 `customAmounts` 欄位。
    - `ExpenseForm` 新增「自訂金額」模式，支援為每位成員手動輸入分擔數值，並具備即時總額校驗。
    - `ExpenseView` 結算邏輯同步更新，優先採用自訂金額計算債務餘額。
  - **色彩體系優化**:
    - 將還款與分帳模式的所有 `sky-blue` 顏色替換為更契合森林主題的 `honey-orange` (蜂蜜橘)。
  - **UI/UX 精進**:
    - 簡化還款按鈕文字為「即刻還款」；在還款模式下啟用描述輸入框。
  - **品質與測試**:
    - 新增 `tests/unit/ExpenseForm.spec.ts`，全面覆蓋模式切換、金額校驗與資料同步邏輯。
    - 修正 `ExpenseForm.vue` 中遺漏的 `ref` 匯入問題。

## [2026-03-07] feat(member): 解放旅伴管理權限與同步編輯功能 (v2.2.7)

- Hash: `TBD`
- 改動方向: 取消移除限制，同步全系統旅伴管理體驗，並優化邊界邏輯。
- 具體內容:
  - **權限解放**: 在 `MemberForm.vue` 與 `TripForm.vue` 中移除了「目前使用者不可移除」的限制，提升名單管理彈性。
  - **功能同步**: 將 `MemberForm.vue` 的旅伴名稱行內編輯邏輯（含自動聚焦、關聯紀錄提示）完整移植至 `TripForm.vue`。
  - **智能適配**: 優化 `ExpenseView.vue`，當目前使用者不在旅伴名單時，自動切換預設付款人為名單首位，並以全體成員分帳。
  - **邊界強化**: `MemberForm.vue` 新增「禁止空名單」驗證與提示；`TripForm.vue` 更新 `isDirty` 偵測逻辑以支持預設使用者移除情境。
  - **測試升級**: 更新 `MemberForm.spec.ts` 與 `TripForm.spec.ts` 測試案例，覆蓋取消限制後的各項邊界情況。

## [2026-03-07] feat(expense): 強化記帳系統：新增還款類型與交易篩選功能 (v2.2.6)

- Hash: `TBD`
- 改動方向: 全面支援還款紀錄與智能分帳路徑，優化交易管理體驗。
- 具體內容:
  - **資料模型**: 更新 `ExpenseSchema` 與 `expenseStore`，新增 `type: 'repayment'`，並在統計時自動過濾還款金額。
  - **分帳邏輯**: `ExpenseView` 分帳結算現可正確抵銷還款債務。
  - **還款捷徑**: 編輯支出時新增「即刻還款」按鈕，自動帶入原支出敘述與建議金額。
  - **篩選功能**: 交易明細新增「全部 / 支出 / 還款」篩選 UI，並優化還款紀錄的視覺樣式。
  - **UI 優化**: 將「旅伴管理」按鈕移至 Header，確保無支出時仍可編輯；統一「新增紀錄」按鈕標籤。
  - **品質保障**: 同步更新 `expenseStore.spec.ts` 測試案例與 `seed.ts` 種子資料以符合新架構。
...
- **Version**: `2.2.0`
- **改動方向**: 在資料收集功能中加入自定義標籤系統，優化靈感的分類與檢索體驗。
- **具體內容**:
  - **資料模型**: 更新 `CollectionSchema` (zod) 加入 `tags` 欄位，並設為預設空陣列以相容舊資料。
  - **Store 邏輯**: 於 `collectionStore` 新增 `allTags` 計算屬性，自動提取所有不重複的標籤。
  - **編輯功能**: 在 `CollectionForm` 加入標籤輸入介面，支援 Enter 新增與點擊移除。
  - **UI 過濾**: 在 `CollectionView` 實作橫向捲動的標籤篩選列，並與來源篩選器同步設計語彙。
  - **驗證程序**: 補充 `CollectionForm.spec.ts` 單元測試，確保標籤新增/移除與自動儲存邏輯正確。
