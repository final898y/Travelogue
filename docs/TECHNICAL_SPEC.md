# 🛠️ Travelogue 技術規格說明書

本文件紀錄專案的技術架構、資料流程與開發規範，供開發者與 AI Agent 參考。
...
├── src/
│   ├── assets/         # 品牌資源、中央圖示庫 (icons.ts)
│   ├── components/
│   │   ├── trip/       # 業務邏輯組件 (Form, Card, Item)
│   │   └── ui/         # 通用基礎 UI (Toast, BottomSheet, Dialog)
│   ├── composables/    # 共享業務邏輯 (useTripDetails)
│   ├── router/         # Vue Router 配置 (History Mode)
│   ├── services/       # Firebase SDK 初始化、種子資料
│   ├── stores/         # Pinia 狀態管理 (auth, trip, plan, expense, ui)
│   ├── types/          # Zod Schema 與 TypeScript 型別定義
│   ├── utils/          # 工具函式 (mapUtils)
│   └── views/          # 頁面級視圖
└── tests/              # Vitest 單元測試與整合測試
```

---

## 2. 狀態管理設計 (Pinia Stores)
